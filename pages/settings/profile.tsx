...
// adding fetch for possible future use;
// need to understand implications on the serverside
...
export default function Profile(props: any) {
    ...
    const {user} = props;
    const {username, name, bio, timeZone, avatar} = user;
    ...
    const updateProfileHandler = async ({username, name, bio, timeZone}: any) => {

        // TODO: Add validation - running the risk of overwriting the info in the original entry

        const response = await fetch(`/api/user/profile`, {
            method: 'PATCH',
            body: JSON.stringify({username, name, description: bio, timeZone}),
            headers: {
        ...
        // const {data, error} = useSWR(`/api/user/profile`, fetch);
        //username=${enteredUsername}&name${enteredName}&description=${enteredDescription}&timeZone=${enteredTimezone}
        // log the error if one exists on stdout
        // if(error) {
        //   console.log(error);
        // } else {
        //   console.log(data);
                        ...
                        <div className="mt-6 flex flex-col lg:flex-row">
                            <div className="flex-grow space-y-6">
                                <div className="flex">
                                    <div className="w-1/2 mr-2">
                                        ...
                                        <div className="mt-1 rounded-md shadow-sm flex">
                                            ...
                                            <input {...register("username")} type="text" name="username" id="username" autoComplete="username" className="focus:ring-blue-500 focus:border-blue-500 flex-grow block w-full min-w-0 rounded-none rounded-r-md sm:text-sm border-gray-300" defaultValue={username} />
                                        </div>
                                    </div>
                                    <div className="w-1/2 ml-2">
                                        <label htmlFor="name" className="block text-sm font-medium text-gray-700">Full name</label>
                                        <input {...register("name")} type="text" name="name" id="name" autoComplete="given-name" placeholder="Your name" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" defaultValue={name} />
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <textarea {...register("bio")} id="about" name="about" placeholder="A little something about yourself." rows={3} className="shadow-sm focus:ring-blue-500 focus:border-blue-500 mt-1 block w-full sm:text-sm border-gray-300 rounded-md">{bio}</textarea>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="mt-1">
                                        <select {...register("timeZone")} name="timezone" id="timeZone" defaultValue={timeZone} className="shadow-sm focus:ring-blue-500 focus:border-blue-500 mt-1 block w-full sm:text-sm border-gray-300 rounded-md">
                                ...
                                <div className="hidden relative rounded-full overflow-hidden lg:block">
                                    {avatar && <img className="relative rounded-full w-40 h-40" src={avatar} alt="" />}
                                    {!avatar && <div className="relative bg-blue-600 rounded-full w-40 h-40"></div>}
